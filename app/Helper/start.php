...
function restaurant()
{
    ...
    $currUser = user();
    if ($currUser && $currUser->restaurant_id) {
        $restaurant = Restaurant::find($currUser->restaurant_id);
...
{
    static $shopBranch = null;
    if ($shopBranch) return $shopBranch;

    if (session()->has('shop_branch')) {
        $shopBranch = session('shop_branch');
        return $shopBranch;
    }

    if (shop()) {
        $shopBranch = shop()->branches->first();
        session(['shop_branch' => $shopBranch]);
        return $shopBranch;
    }
...
function currency()
{
    ...
    $res = restaurant();
    if ($res) {
        $currency = $res->currency->currency_symbol;
...
if (!function_exists('global_setting')) {
    ...
    function global_setting()
    {
        ...
        if (cache()->has('global_setting')) {
            $globalSetting = cache('global_setting');
            return $globalSetting;
        }

        $globalSetting = GlobalSetting::first();
        cache(['global_setting' => $globalSetting]);
...
function superadminPaymentGateway()
{
    ...
    if (cache()->has('superadminPaymentGateway')) {
        $payment = cache('superadminPaymentGateway');
        return $payment;
    }
    ...
    function currency_format($amount, $currencyId = null, $showSymbol = true, $showCode = false)
    {
        ...
        if ($showCode) {
            ...
            if (!$showSymbol) {
                ...
                $settings = restaurant();
                $currency_symbol = ($currencyId == null && $settings) ? $settings->currency->currency_symbol : $formats->currency_symbol;
            }
        }
