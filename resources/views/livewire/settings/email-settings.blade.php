    ...
    <form wire:submit="submitForm" class="space-y-6">
        ...
        <div class="grid gap-6 lg:grid-cols-4">
            ...
            <div>
                ...
                <x-input type='text' id="mailFromName" class="mt-1 block w-full" wire:model.defer="mailFromName"/>
                <x-input-error for="mailFromName" class="mt-2"/>
            </div>

            <div>
                <x-label for="mailFromEmail" :value="__('modules.settings.mailFromEmail')"/>
                <x-input type="text" id="mailFromEmail" class="mt-1 block w-full" wire:model.defer="mailFromEmail"/>
                <x-input-error for="mailFromEmail" class="mt-2"/>
            </div>

            <div>
                <x-label for="enableQueue" :value="__('modules.settings.enableQueue')"/>
                <x-select id="enableQueue" class="mt-1 block w-full" wire:model.defer="enableQueue">
        ...
        @if ($mailDriver == 'smtp')
            <div class="grid gap-6 lg:grid-cols-4">
                <div>
                    ...
                    <x-input type='text' id="smtpHost" class="mt-1 block w-full" wire:model.defer="smtpHost"/>
                    <x-input-error for="smtpHost" class="mt-2"/>
                </div>
                <div>
                    <x-label for="smtpPort" :value="__('modules.settings.smtpPort')"/>
                    <x-input type='text' id="smtpPort" class="mt-1 block w-full" wire:model.defer="smtpPort"/>
                    <x-input-error for="smtpPort" class="mt-2"/>
                </div>
                <div>
                    <x-label for="mailUsername" :value="__('modules.settings.mailUsername')"/>
                    <x-input type='text' id="mailUsername" class="mt-1 block w-full" wire:model.defer="mailUsername"/>
                    <x-input-error for="mailUsername" class="mt-2"/>
                </div>
                <div>
                    <x-label for="mailPassword" :value="__('modules.settings.mailPassword')"/>
                    <x-input-password type='password' id="mailPassword" class="mt-1 block w-full" wire:model.defer="mailPassword"/>
